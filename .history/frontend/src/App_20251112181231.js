...
const styles = {
  ...
  formGroup: {
    display: 'flex',
    flexDirection: 'column',
    marginBottom: '20px', // --- NEW --- (Added margin for spacing)
  },
  label: {
    marginBottom: '8px',
    fontSize: '0.9rem',
  ...
  // --- NEW --- (Style for the new dropdown)
  select: {
    padding: '14px',
    fontSize: '1.1rem',
    borderRadius: '10px',
  ...
  // --- NEW --- (Style for the new recommendation box)
  recommendationValue: {
    fontSize: '1.5rem',
    fontWeight: 'bold',
...
function App() {
  ...
  // --- NEW ---
  // Add state to track the selected activity
  const [activity, setActivity] = useState('run'); // Default to 'run'
  ...
  const handleSubmit = (e) => {
    ...
    // --- NEW ---
    // Send both the city and the selected activity to the backend
    const dataToSend = { 
      city: location,
      activity: activity // Add the activity state
    };
        ...
        <div style={styles.resultsContainer}>
          ...
          {!isLoading && predictionData && (
            <div style={styles.gridContainer}>
              ...
              {/* --- NEW: Recommendation Box (Top & Highlighted) --- */}
              <div style={{
                  ...styles.resultBox, 
                  ...styles.fullWidthItem,
                  border: '2px solid #f6e05e', // <-- HIGHLIGHT BORDER
                  boxShadow: '0 4px 20px 0 rgba(246, 224, 94, 0.15)' // <-- HIGHLIGHT GLOW
                }}>
